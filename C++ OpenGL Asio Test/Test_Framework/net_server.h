...
namespace olc {
namespace net {
//enum class DirectionTypes : uint32_t {
//  UP,
//  DOWN,
//  RIGHT,
//  LEFT,
...
public:
    ...
    m_asioAcceptor.async_accept(
      ...
      if (!ec) {
        ...
        if (OnClientConnect(newconn)) {
          ...
          std::cout << "[" << m_deqConnections.back()->GetID() << "] ConnectionApproved\n";
        } else {
  ...
  void MessageGameData(const message<T>& msg, std::shared_ptr<connection<T>> pIgnoreClient = nullptr) {
    bool bInvalidClientExists = false;
    for (auto& client : m_deqConnections) {
      if (client && client->IsConnected()) {
        if (client != pIgnoreClient) {
