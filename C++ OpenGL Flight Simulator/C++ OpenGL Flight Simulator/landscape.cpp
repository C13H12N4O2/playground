...
void Landscape::create(double rug) {
  ...
  for (size_t i = 0; i != rows; ++i) {
    for (size_t j = 0; j != columns; ++j) {
      if (d[i][j] < 0) d[i][j] = 0;
      if (d[i][j] > hightest) hightest = d[i][j];
    }
  }
...
void Landscape::generate(int x1, int y1, int x2, int y2, double rug) {
  ...
  if (x3 < x2 && y3 < y2) {
    d[x3][y3] = (d[x1][y1] + d[x2][y1] + d[x1][y2] + d[x2][y2]) / 4 + scale(rug * (fabs(static_cast<double>(x2 - x1)) + fabs(static_cast<double>(y2 - y1))));
  }
...
void Landscape::vertex(double x, double z) {
  double y = d[static_cast<int>(x)][static_cast<int>(z)];
  double h = y / hightest;
...
void Landscape::drawTriangle(int x1, int z1, int x2, int z2, int x3, int z3) {
  ...
  if (d[x1][z1] < h && d[x2][z2] < h && d[x3][z3] < h) {
    ...
    glNormal3d(0, 1.0, 0);
    glVertex3dv(reinterpret_cast<GLdouble *>(&p[0]));
    glVertex3dv(reinterpret_cast<GLdouble *>(&p[1]));
    glVertex3dv(reinterpret_cast<GLdouble *>(&p[2]));
  } else {
    glNormal3dv(reinterpret_cast<GLdouble *>(&normal));
    vertex(x1, z1);
    vertex(x2, z2);
    vertex(x3, z3);
  }
